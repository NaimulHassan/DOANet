...
for i in np.arange(4):
    ...
    Elevation_err = np.asarray(
        np.abs(np.reshape(Actual_elevation, (-1, 1)) - np.reshape(predicted_elevation, (-1, 1)))
        * 180.0,
        dtype=np.float64,
    )
...
pred_filepath = "predictions_fibo_elevation_2367_noisy_competition_train.mat"
savemat(
    pred_filepath,
    dict(
        [
            (
                "static_elevation",
                np.swapaxes(np.asarray(Actual_elevation_list, dtype=np.float64) * 180.0, 0, 1),
            ),
            (
                "predicted_static_elevation",
                np.squeeze(
                    np.swapaxes(
                        np.asarray(predicted_elevation_list, dtype=np.float64) * 180.0, 0, 1
                    ),
                    axis=-1,
                ),
            ),
            (
                "elevation_error",
                np.swapaxes(
                    np.squeeze(np.asarray(Elevation_error_list, dtype=np.float64), axis=-1), 0, 1
                ),
            ),
        ]
    ),
