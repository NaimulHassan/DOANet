...
#    conv7 = MaxPooling1D(pool_size=2, padding='valid', name='conv7_maxpool')(conv7)
    ...
    conv8 = Conv1D(filters=2048, kernel_size=9, padding='valid', dilation_rate=34, activation='relu',
    ...
    conv9 = Conv1D(filters=2048, kernel_size=7, padding='valid', dilation_rate=55, activation='relu',
    ...
    azi_block = Dense(1024, activation='relu', name='azi_dense1')(flatten_block)
    azi_block = BatchNormalization(name='azi_dense1_bn')(azi_block)
    azi = Dense(2, activation='tanh', name='azi')(azi_block)

    model = Model(inputs=input_layer, outputs=azi)
...
trained_model_path = "../input/static-task-models/best_azi_2367.hdf5"
...
for i in np.arange(4):
    ...
    #%% Prediction
    predicted_azimuth, predicted_elevation, prediction_data_index, Actual_xy, Actual_azimuth, Actual_elevation = [],[],[],[],[],[]
    for i in range(int(np.ceil(val_sample_size/val_data_batch))):
#    for i in range(2):
        wav, _, A_azimuth, A_elevation, index = test_gen.__getitem__(i)
        p_xy = model.predict(wav)
        p_azimuth = np.arctan2(np.asarray(p_xy)[:,1], np.asarray(p_xy)[:,0])/np.pi
        Actual_azimuth.extend(A_azimuth)
        predicted_azimuth.extend(p_azimuth)
        del wav, index, p_xy, A_azimuth, p_azimuth, A_elevation
        
    #%% Error calculation
        
    Azimuth_err = np.asarray(np.abs(np.reshape(Actual_azimuth, (-1,1)) - np.reshape(predicted_azimuth, (-1,1)))*180.0, dtype = np.float64)
#    Elevation_err = np.asarray(np.abs(np.reshape(Actual_elevation, (-1,1)) - np.reshape(predicted_elevation, (-1,1)))*180.0, dtype = np.float64)
    np.place(Azimuth_err, Azimuth_err>180.0, 360.0 - Azimuth_err[Azimuth_err>180.0])
#    np.place(Elevation_err, Elevation_err>180.0, 360.0 - Elevation_err[Elevation_err>180.0])
    
    Azimuth_error_list.append(Azimuth_err)
    predicted_azimuth_list.append(predicted_azimuth)
    Actual_azimuth_list.append(Actual_azimuth)
    del Azimuth_err, predicted_azimuth, Actual_azimuth
#%% save predictions in a .mat file

pred_filepath = 'predictions_fibo_azi_2367_noisy_competition_train.mat'
savemat(pred_filepath, dict([#('static_azimuth', np.asarray(np.reshape(Actual_azimuth, (-1, 1))*180.0, dtype = np.float64)), 
                             ('static_azimuth', np.swapaxes(np.asarray(Actual_azimuth_list, dtype = np.float64)*180.0, 0, 1)),
                             #('predicted_static_azimuth', np.asarray(np.reshape(predicted_azimuth, (-1, 1))*180.0, dtype = np.float64)),
                             ('predicted_static_azimuth', np.swapaxes(np.asarray(predicted_azimuth_list, dtype = np.float64)*180.0, 0, 1)),
                             #('azimuth_error', Azimuth_err),
                             ('azimuth_error', np.swapaxes(np.squeeze(np.asarray(Azimuth_error_list, dtype = np.float64), axis = -1), 0, 1))]))