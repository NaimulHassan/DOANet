...
test_gen = DataGenerator(
    sample_size=val_sample_size, data_source=db_val, batch_size=val_data_batch
)
...
for i in np.arange(4):
    ...
    Azimuth_err = np.asarray(
        np.abs(np.reshape(Actual_azimuth, (-1, 1)) - np.reshape(predicted_azimuth, (-1, 1)))
        * 180.0,
        dtype=np.float64,
    )
...
savemat(
    ...
    dict(
        [
            (
                "static_azimuth",
                np.swapaxes(np.asarray(Actual_azimuth_list, dtype=np.float64) * 180.0, 0, 1),
            ),
            (
                "predicted_static_azimuth",
                np.swapaxes(np.asarray(predicted_azimuth_list, dtype=np.float64) * 180.0, 0, 1),
            ),
            (
                "azimuth_error",
                np.swapaxes(
                    np.squeeze(np.asarray(Azimuth_error_list, dtype=np.float64), axis=-1), 0, 1
                ),
            ),
        ]
    ),
