...
def CNN_fibo(params):
    ...
    conv1 = Conv1D(filters=32, kernel_size=21, padding='valid', dilation_rate=1, activation='relu',
                   kernel_initializer='he_normal', name='conv1')(extracted_channels)
...
#    elev = Dense(1, activation='tanh', name='elev')(elev_block)
...
pretrained_model_path = "../input/in-flight-speech-2367-azimuth-model/best_azi_2367.hdf5"
pretrained_logfile = "../input/in-flight-speech-2367-azimuth-model/trainingLog_2367_azi.csv"
...
train_filepath = "../input/dnn-prefilter-unet/speech_flight_train.hdf5"
val_filepath = "../input/dnn-prefilter-unet/speech_flight_val.hdf5"
...
train_gen = DataGenerator(sample_size = train_sample_size, data_source = db_train, batch_size = train_data_batch)
val_gen = DataGenerator(sample_size = val_sample_size, data_source = db_val, batch_size = val_data_batch)

#%% model path
modelWeightPath = 'best_azi_2367.hdf5'
modelLogPath = 'trainingLog_2367_azi.csv'
...
trainingCallbacks.append(ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=5, verbose=1, min_lr=1e-10, mode='auto', min_delta=1e-4, cooldown=0))
trainingCallbacks.append(CSVLogger(modelLogPath, separator=',', append=True))

#%% starting training
model.fit_generator(generator = train_gen, 
                    validation_data=val_gen,
                    epochs=200, verbose=1, initial_epoch=0,
